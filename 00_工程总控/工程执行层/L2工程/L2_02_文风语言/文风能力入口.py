...
from 文风修复规划 import 模块细节, 规划文风修复
from 文风诊断 import 执行文风诊断
...
def 安全生成修复单(
    ...
    *,
    ...
    try:
        ...
        ctx = 构造文风上下文(resolved, item, repo_root=repo_root)
        parsed, diagnosis = 执行文风诊断(ctx, item, client=client)
        validated, base_errors = 校验通用证据引用(parsed, ctx.正文语料)
        errors = base_errors + 校验文风响应(parsed, ctx.正文语料, diagnosis)
        if errors:
            raise 能力诊断错误("；".join(errors[:6]), kind="EVIDENCE_INVALID")
        plan = 规划文风修复(diagnosis, parsed)
        form = 诊断计划转修复单(
            item,
            rules,
            module_id=MODULE_ID,
