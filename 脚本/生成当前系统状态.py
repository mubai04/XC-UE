...
GENERATED_STATUS_PATH = ROOT / "00_工程总控" / "当前系统状态_自动生成.md"
...
def build_status_markdown(*, refresh_tests: bool = False) -> str:
    if refresh_tests:
        subprocess.run(
            [sys.executable, str(ROOT / "脚本" / "冻结审计基线.py"), "--skip-pytest-run"],
            cwd=ROOT,
            check=False,
        )
        subprocess.run(
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="生成 当前系统状态_自动生成.md")
    parser.add_argument(
        "--refresh-tests",
        action="store_true",
