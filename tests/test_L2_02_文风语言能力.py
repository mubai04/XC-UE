...
def test_l2_02_tone_evidence_insufficient_skips_dual_requirement(tmp_path):
    text = "赵六说：我只说这一句。"
    path = tmp_path / "insufficient.md"
    path.write_text(text, encoding="utf-8")
    ctx = 构造文风上下文(path, _item("z", "我只说这一句"))
    insufficient = [c for c in ctx.语气比较候选 if c.get("status") == "evidence_insufficient"]
    assert insufficient
    parsed = {
        "root_cause": "语气",
        "style_issues": [
            {
                ...
                "source_a": {"paragraph": 1, "quote": "赵六说：我只说这一句。"},
            }
        ],
        "fix_actions": ["暂不判定语气漂移"],
        "acceptance_criteria": ["补第二处说话证据"],
        "evidence_quotes": [{"paragraph": 1, "quote": "赵六说：我只说这一句。"}],
        "forbid_modify_scope": "事件顺序、人物目标、世界规则",
    }
    diag = 文风诊断结果("语气", [])
    errors = 校验文风响应(parsed, ctx.正文语料, diag)
    assert not any("缺少 source_a/source_b" in e for e in errors)


def test_l2_02_validator_accepts_valid_dual_source(tmp_path):
    q1 = "钱七说：快走，别回头。"
    q2 = "钱七问：你们还在犹豫什么？"
    text = f"{q1}\n\n{q2}"
    path = tmp_path / "dual.md"
    path.write_text(text, encoding="utf-8")
    ctx = 构造文风上下文(path, _item("q", q1))
    parsed = {
        "root_cause": "语气漂移",
        "style_issues": [
            {
                ...
                "difference": "命令句与疑问句语气不一致",
            }
        ],
        "fix_actions": ["统一钱七两处语气强度"],
        "acceptance_criteria": ["两处语气可核对"],
        "evidence_quotes": [{"paragraph": 1, "quote": q1}],
        "forbid_modify_scope": "事件顺序、人物目标、世界规则",
    }
    diag = 文风诊断结果("语气", [文风问题("人物语气漂移", 1, 1, q1, "统一语气")])
    errors = 校验文风响应(parsed, ctx.正文语料, diag)
    assert not any("语气漂移" in e and "quote" in e for e in errors)
