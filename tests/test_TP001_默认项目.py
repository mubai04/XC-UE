...
from tests.test_端到端流水线 import (
    _e2e_workspace,
    _l2_01_pipeline_chapter,
...
def test_tp001_l1_l2_l3_inprocess_mock(monkeypatch, repo_root: Path):
    ...
    focused = [item for item in packet.get("items", []) if item.get("失败类型") == "叙事失败"]
    assert focused, "L1 未产出叙事失败项"
    ...
    from tests.test_端到端流水线 import _import_l15_main, _run_entry as e2e_run_entry, _run_l15_on_packet
