...
def test_l2_03_no_default_goal_when_text_lacks_goal_evidence(tmp_path):
    path = tmp_path / "no_goal.md"
    path.write_text("天气转阴。远处有鸟飞过。", encoding="utf-8")
    ctx = 构造角色上下文(path, _item("g", "天气"))
    blob = str(ctx.角色状态表) + str(ctx.目标刺激行为链)
    assert "生存/达成目标" not in blob
    assert "规则正在收紧" not in blob
    for state in ctx.角色状态表:
        assert not state.当前目标 or state.当前目标 in path.read_text(encoding="utf-8")


def test_l2_03_rename_preserves_extracted_chain(tmp_path):
    hero = f"韩{uuid.uuid4().hex[:2]}"
    body = (
        f"{hero}想夺回旧物。号炮齐响后，{hero}奔向关隘。"
        f"敌哨拦住他，他弃下包袱。"
    )
    path = tmp_path / "rename.md"
    path.write_text(body, encoding="utf-8")
    ctx = 构造角色上下文(path, _item("r", body[:8]))
    chain = next(c for c in ctx.目标刺激行为链 if c.get("character") == hero)
    goal_ev = (chain.get("goal_evidence") or chain.get("goal") or "").strip()
    assert goal_ev and goal_ev in body
