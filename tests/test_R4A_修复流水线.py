...
from tests.conftest import ROOT, failure_packet_item, failure_packet_payload, l2_04_v2_payload, l2_06_v2_payload, make_mock_transport, sample_chapter_text
        ...
        "consistency_conflicts": [
            {
                ...
                "entity": "空间层级",
                "attribute": "层级",
                "source_a": {"paragraph": 9, "quote": quote, "source_type": "正文"},
                "source_b": {"paragraph": 10, "quote": quote_b, "source_type": "正文"},
                "classification": "硬冲突",
            }
        ],
        "fix_actions": ["统一事实"],
        "acceptance_criteria": ["无冲突"],
        "evidence_quotes": [{"paragraph": 9, "quote": quote}],
        "needs_reroute": False,
    }


@pytest.mark.parametrize(
    "failure_type,source_gate,module,payload_fn,run_suffix",
    [
        ("创意设定失败", "L1-01", "L2-04", lambda q: _setting_payload(q), "p04"),
        ("E低：即时情绪反馈弱", "L1-02", "L2-05", lambda q: _market_payload(q), "p05"),
        ("前后事实冲突", "L1-01", "L2-06", lambda q: _consistency_payload("空间层级仍标记为上层", "空间层级已经进入下层"), "p06"),
    ],
)
def test_repair_pipeline_independent_module_paths(
    pipeline_workspace, monkeypatch, repo_root, failure_type, source_gate, module, payload_fn, run_suffix
):
    workspace, seed, chapter, harness = pipeline_workspace
    pipeline_id = f"R4A-{run_suffix}-{seed}"
    packet_path = workspace / f"{run_suffix}.json"
    quote = "忽然察觉异常，因为规则正在收紧"
    packet_chapter = chapter
    if module == "L2-06":
        chapter.write_text(
    ...
    if module == "L2-04":
        packet_chapter = harness / "chapters" / "ch01.md"
        ir = harness / "IR"
        ...
        packet_chapter.write_text(
            packet_chapter.read_text(encoding="utf-8")
            + "\n\n正文未写：不持牌者不得入塔，也未写持牌代价。\n\n仍无具体后果。",
            encoding="utf-8",
        )
    packet_path.write_text(
        json.dumps(
            _packet(packet_chapter, seed, failure_type, module, pipeline_id, source_gate),
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
