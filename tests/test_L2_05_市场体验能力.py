...
def test_l2_05_repeat_uses_sentence_fragments_not_comma_chunks(tmp_path):
    phrase = "同一警告语在两端重复出现"
    text = f"段落一：{phrase}，危险逼近。\n\n段落二：中段推进。\n\n段落三：{phrase}，仍未解释。"
    path = tmp_path / "repeat.md"
    path.write_text(text, encoding="utf-8")
    ctx = 构造阅读阶段上下文(path, _item("r", phrase))
    assert ctx.重复信息
    for rep in ctx.重复信息:
        assert rep.位置A != rep.位置B


def test_l2_05_empty_chapter_yields_no_fabricated_candidates(tmp_path):
    path = tmp_path / "empty.md"
    path.write_text("", encoding="utf-8")
    ctx = 构造阅读阶段上下文(path, _item("e", ""))
    assert ctx.阅读阶段表 == [] or all(s.文本摘要 == "" or not s.文本摘要 for s in ctx.阅读阶段表)
    entrance = getattr(ctx, "入口承诺候选", None)
