...
from tests.conftest import ROOT, failure_packet_item, failure_packet_payload, find_chapter_evidence, make_mock_transport, make_semantic_context, sample_chapter_text, semantic_audit_payload
...
def _l2_01_pipeline_chapter(seed: str) -> str:
    return _failure_chapter(seed)
...
def test_l1_l2_l3_entry_mock_pipeline(monkeypatch, repo_root):
    ...
    chapter.write_text(_l2_01_pipeline_chapter(seed), encoding="utf-8")
    ...
    _original_mark_diagnostic = L1决策角色.标记诊断项

    def _force_blocking_diagnostic(item):
        was_blocking_failure = item.严重级别 == "error" or item.状态 == "失败"
        item = _original_mark_diagnostic(item)
        if was_blocking_failure:
            item.decision_role = L1决策角色.内容决策角色
            item.blocking = True
            item.routeable = True
    ...
    monkeypatch.setattr(L1决策角色, "标记诊断项", _force_blocking_diagnostic)
    ...
    focused_items = [item for item in packet.get("items", []) if item.get("失败类型") == "叙事失败"]
    assert focused_items, "L1 未产出叙事失败项"
    packet["items"] = focused_items[:1]
    packet["failure_count"] = len(packet["items"])
    filtered_packet_path = workspace / f"{seed}_l15_input.json"
    ...
    assert l15_payload.get("target_module") == "L2-01"
...
def test_l2_fictional_evidence_blocks_l3_entry(monkeypatch, repo_root):
    ...
    packet["items"][0]["名称"] = "有序叙事信号"
    packet["items"][0]["说明"] = f"{seed} 结构链断"
    packet["items"][0]["候选模块"] = "L2-01"
    packet["items"][0]["修复方向"] = "压缩主行动线"
