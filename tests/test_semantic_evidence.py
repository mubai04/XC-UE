...
def test_paragraph_id_repaired_when_exact_text_unique_in_scope():
    corpus = {
        "P0001": "标题",
    ...
    result = 校验语义审计响应(parsed, current_paragraphs=corpus)
    assert result.ok
    assert any("PARAGRAPH_ID_REPAIRED" in w for w in result.warnings)
    assert result.validated_evidence[0].paragraph_id == "P0002"


def test_reject_more_than_one_evidence_item():
    ...
    result = 校验语义审计响应(parsed, current_paragraphs=corpus)
    assert not result.ok
    assert any("最多 1 条" in e for e in result.errors)
...
def test_causal_pass_requires_chain_in_final_reason():
    corpus = {"P0001": "他必须做出选择，否则代价会落在所有人身上。"}
    payload = _full_payload("他必须做出选择")
    for dim in payload["dimensions"]:
        if dim["name"] == "因果":
            dim["verdict"] = "PASS"
            dim["final_reason"] = "全章因果成立，事件增量明显。"
    result = 校验语义审计响应(payload, current_paragraphs=corpus)
    assert not result.ok
    assert any("起因" in e and "因果" in e for e in result.errors)
